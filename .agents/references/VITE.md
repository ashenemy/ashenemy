...
- `"paths": { "~/*": ["./app/*"] }` — self-referential alias; keep as-is for a single-project app.
...
7. Remove redundant npm scripts so `@nx.vite` infers them natively (see "Redundant npm Scripts" section)
...
6. Add `extends` to solution-style tsconfigs so root settings apply
7. Remove redundant npm scripts — `@nx/vite/plugin` infers `build`, `dev`, `preview`, `test` (see "Redundant npm Scripts" above)
...
- Post-import fixes:
  1. Removed stale `node_modules/`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`, `.gitignore` from each package
